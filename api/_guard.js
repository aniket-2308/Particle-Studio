...
// Bundled starter models shipped with the app (served from our own origin, fixed
// paths). Embeds of the "starter" cloud point here instead of a Blob upload.
const BUNDLED_MODEL_PATHS = new Set(['/assets/Earth_1_12756.glb']);

// A saved scene's model_url must be one of OUR Vercel Blob files OR a bundled
// starter asset (https only). Blocks pointing embeds at arbitrary third-party URLs.
export function isAllowedModelUrl(url) {
  ...
  // Bundled starter: exact known path only (constant — no SSRF surface).
  return BUNDLED_MODEL_PATHS.has(u.pathname);
}
